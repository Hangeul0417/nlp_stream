...
def main():
    ...
    menu = st.sidebar.selectbox("메뉴", ["텍스트 분석", "감성 분석", "번역", " 이 프로젝트에"])

    if menu == "텍스트 분석":
        raw_text = st.text_area("TEXT INPUT", "여기에 영어텍스트 입력", height = 300)
        if st.button("시작"):
                if len(raw_text) == 0:
                    st.warning("텍스트! 넣으라고! 했잖아요! 안넣으면! 제가! 뭘해요!")
                else:
        ...
        if len(raw_text) < 3:
             st.warning("텍스트가! 짧아요! 좀 더 길게!")
        else:
             target_language = st.selectbox("번역할 언어를 선택하세요", ["German", "Spanish", "French", "Italian", "Japanese"])

             if target_language == "Germen":
                  target_lang = "de"
             elif target_language == "Spanish":
                  target_lang = "es"
             elif target_language == "Fench":
                  target_lang = "fr"
